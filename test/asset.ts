...
describe('asset', function() {
    ...
    it('should create from string', function() {
        ...
        assert.equal(oneBex.amount, 1)
        assert.equal(oneBex.symbol, 'BEX')
    ...
    it('should parse price', function() {
        const price1 = new Price(Asset.from('1.000 BEX'), Asset.from(1, 'BBD'))
        const price2 = Price.from(price1)
        const price3 = Price.from({base: '1.000 BEX', quote: price1.quote})
        assert.equal(price1.toString(), '1.000 BEX:1.000 BBD')
    ...
    it('should get vesting share price', function() {
        const props: any = {
            total_vesting_fund_dpay: '5.000 BEX',
        ...
        assert.equal(price1.quote.symbol, 'BEX')
        const badProps: any = {
            total_vesting_fund_dpay: '0.000 BEX',
        ...
        assert.equal(price2.quote.symbol, 'BEX')
    })

    it('should convert price', function() {
        const price1 = new Price(Asset.from('0.500 BEX'), Asset.from('1.000 BBD'))
        const v1 = price1.convert(Asset.from('1.000 BEX'))
        assert.equal(v1.amount, 2)
        assert.equal(v1.symbol, 'BBD')
        const v2 = price1.convert(Asset.from('1.000 BBD'))
        assert.equal(v2.amount, 0.5)
        assert.equal(v2.symbol, 'BEX')
        assert.throws(() => {
